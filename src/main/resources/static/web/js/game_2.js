...
function loadData() {
  $.get('/api/game_view/' + getParameterByName('gp'))
    .done(function (data) {
      ...
      data.ships.forEach(function (elemento) {
        console.log(elemento.locations)
        elemento.locations.forEach(function (location) {
          var turnHitted = isHit(location,data.salvoes,playerInfo[0].id)
          if(turnHitted >0){
            $('#B_' + location).addClass('ship-piece-hited');
            $('#B_' + location).text(turnHitted);
          }
          else
            $('#B_' + location).addClass('ship-piece');
        });
      });
